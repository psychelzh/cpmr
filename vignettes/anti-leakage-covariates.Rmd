...
Use `fit(cpm_spec(...), ...)` for a single fit and `fit_resamples()` for CV.
...
- For `fit()`, use `return_edges = TRUE` to store selected edges, or `FALSE`
  when you only need predictions/performance.
- For `fit_resamples()`, use `return_edges = "sum"` to keep fold-aggregated edge
  counts.
- Use `collect_edges(..., format = "index")` to export sparse indices.
- For `fit_resamples()`, use `return_edges = "none"` if you only need predictive
  performance.
