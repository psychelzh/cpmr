...
# Leakage-safe CPM call: covariates are handled fold-wise inside assessment
res <- cpm(
  conmat = conmat,
  behav = behav,
  spec = spec(),
...
summary(res_with_edges)
head(res_with_edges$predictions)
dim(res_with_edges$edges)
```

Using a reusable `spec()` object is helpful when you want the same CPM
settings to drive multiple analyses under different assessment plans.
