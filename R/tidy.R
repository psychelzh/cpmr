...
#'   For `component = "edges"`:
#'
#'   \item{pos}{A logical vector of selected edges (positive).}
#'
#'   \item{neg}{A logical vector of selected edges (negative).}
#' @export
    ...
    edges = {
      if (is.null(sum_x$edges)) {
        warning(
          ...
          "* You probably called `fit(cpm_spec(...), ..., return_edges = FALSE)`."
        )
