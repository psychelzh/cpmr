...
test_that("resample validation covers error paths and warnings", {
  ...
  expect_equal(validate_kfolds(3), 3L)
  expect_error(
    validate_kfolds(1),
  ...
  expect_error(
    validate_resamples("bad", include_cases),
    "`resamples` must be a non-empty list of assessment indices.",
    fixed = TRUE
  )
  expect_error(
    validate_resamples(list(1:2), include_cases),
    "must contain at least 2 assessment sets",
    fixed = FALSE
  )
  expect_error(
    validate_resamples(list(c(1, NA), 3:6), include_cases),
    "finite numeric indices",
    fixed = FALSE
  )
  expect_error(
    validate_resamples(list(c(1, 2.5), 3:6), include_cases),
    "integer-valued indices",
    fixed = FALSE
  )
  expect_error(
    validate_resamples(list(c(0, 1), 2:6), include_cases),
    "positive indices",
    fixed = FALSE
  )
  expect_error(
    validate_resamples(list(c(1, 1), 2:6), include_cases),
    "must not contain duplicates",
    fixed = FALSE
  )
  expect_error(
    validate_resamples(list(1:3, 4:7), include_cases),
    "contained in complete-case rows",
    fixed = FALSE
  )
  expect_error(
    validate_resamples(list(1:3, 3:6), include_cases),
    "must not overlap across folds",
    fixed = FALSE
  )
  expect_error(
    validate_resamples(list(1:2, 3:4), include_cases),
    "must cover all complete-case rows exactly once",
    fixed = FALSE
  )

  expect_error(
    resolve_resample_folds(list(1:3, 4:6), 2, include_cases),
    "Specify either `resamples` or `kfolds`, not both.",
    fixed = TRUE
  )
  expect_error(
    resolve_resample_folds(NULL, 7, include_cases),
    "`kfolds` must be less than or equal to complete-case observations.",
    fixed = TRUE
  )
  expect_error(
    resolve_resample_folds(list(1:4, 5:6), NULL, include_cases),
    "leave at least 3 complete-case training observations",
    fixed = FALSE
  )
  expect_warning(
    warn_large_edge_storage(700000, 2, "all"),
    "large memory",
    fixed = FALSE
  )
})
...
test_that("core_fit_resamples covers edge-case failures and evaluation helpers", {
  ...
  expect_error(
    validate_resamples(list(1:2, 2:3), include_cases = 1:3),
    "must not overlap",
    fixed = FALSE
  )
  ...
  expect_equal(safe_cor(c(1), c(1)), NA_real_)
  expect_equal(safe_cor(c(1, 1, 1), c(1, 2, 3)), NA_real_)
})
